...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(aNotification: NSNotification) {
        ...
        let currentProcessID = "\(NSProcessInfo.processInfo().processIdentifier)"
        
        // Get all running process IDs with the exact name Darken
        var PIDs = run("/usr/bin/pgrep -x Darken & echo ''").read()
        
        // Split to array
        var PIDArray = split(PIDs) {$0 == "\n"}
        
        var i = 1
        for PID in PIDArray {
            // Kill to process if it's not the current App process id
